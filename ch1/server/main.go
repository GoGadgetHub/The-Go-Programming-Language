package main

import (
	"fmt"
	"image"
	"image/color"
	"image/gif"
	"io"
	"log"
	"math"
	"math/rand"
	"net/http"
	"strconv"
	"sync"
)

var count int
var mu sync.Mutex

func main() {
	http.HandleFunc("/", handle)
	http.HandleFunc("/count", countHandle)
	http.HandleFunc("/requestInfo", requestHandle)
	http.HandleFunc("/gif", gifHandle)
	log.Fatal(http.ListenAndServe("localhost:8080", nil))
}

func gifHandle(writer http.ResponseWriter, request *http.Request) {
	if err := request.ParseForm(); err != nil {
		log.Fatal(err)
	}

	for k, v := range request.Form {
		fmt.Fprintf(writer, "%s: %s\n", k, v)
	}
	cycle, _ := strconv.Atoi(request.Form["cycle"][0])

	lissajous(writer, float64(cycle))
}

func requestHandle(writer http.ResponseWriter, request *http.Request) {
	fmt.Fprintf(writer, "%s, %s, %s\n", request.Method, request.URL, request.Host)
	for k, v := range request.Header {
		fmt.Fprintf(writer, "%s: %v\n", k, v)
	}
	fmt.Fprintf(writer, "%s\n", request.RemoteAddr)

	if err := request.ParseForm(); err != nil {
		log.Fatal(err)
	}

	for k, v := range request.Form {
		fmt.Fprintf(writer, "%s: %s\n", k, v)
	}
}

func countHandle(writer http.ResponseWriter, request *http.Request) {
	mu.Lock()
	defer mu.Unlock()
	fmt.Fprintf(writer, "%d", count)
}

/* this func call twice, because '/' and "/favicon.ico" */
func handle(writer http.ResponseWriter, request *http.Request) {
	// 预防竞态bug
	fmt.Printf("Method: %s, Path: %s\n", request.Method, request.URL.Path)
	mu.Lock()
	count++
	fmt.Printf("why count increase 2 \n")
	mu.Unlock()
	fmt.Fprintf(writer, "%q", request.URL.Path)
}

var palette = []color.Color{color.White, color.RGBA{255, 0, 0, 255}}

const (
	whiteIndex = 0
	redIndex   = 1
)

func lissajous(out io.Writer, cycles float64) {
	const (
		res     = 0.001 // 角度分辨率
		size    = 100   // 画布包含 [-size, +size]
		nframes = 64    // 帧数
		delay   = 8     // 10ms单位的帧间延迟
	)

	freq := rand.Float64() * 3.0 // y震荡器的相对频率
	anim := gif.GIF{LoopCount: nframes}
	phase := 0.0
	for i := 0; i < nframes; i++ {
		rect := image.Rect(0, 0, 2*size+1, 2*size+1) // 内容容器
		img := image.NewPaletted(rect, palette)      //
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			x := math.Sin(t)
			y := math.Cos(t) + math.Sin(t*freq+phase)
			if y >= 0 {
				img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), redIndex)
				img.SetColorIndex(size+int(x*size+0.5), size+int(-y*size+0.5), redIndex)
			}
		}
		phase += 0.1
		anim.Delay = append(anim.Delay, delay)
		anim.Image = append(anim.Image, img)
	}
	gif.EncodeAll(out, &anim)
}
